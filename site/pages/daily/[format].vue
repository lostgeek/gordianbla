  ...
  <Splitter :layout="getLayout">
    <SplitterPanel class="left" :size="75">
      <template v-if="loaded">
        ...
        <CardInputField :class="(gordian.finished.value) ? 'hidden' : null" :cards="cards" @submit="(card) => gordian.guess(card)" />
      </template>
      <template v-else>
        <GuessTable
    ...
    <SplitterPanel class="right" :size="25">
      ...
      <template v-if="loaded">
        <Puzzle
          v-if="cardSvg" :puzzle-mode="puzzleMode" :reveal-level="revealLevel" :card-url="cardUrl"
          :card-svg="cardSvg"
        />
      </template>
      <template v-else>
...
watch(gordian.guesses, async (newG, oldG) => {
  ...
  if (gordian.finished.value) {
    if (oldG.length > 0) { // If this is not the page loading to a finished puzzle
      ...
      const formatBeforeTimeout = format.value
      setTimeout(() => {
        const route = useRoute()
