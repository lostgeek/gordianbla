export const useUser = defineStore("userStore", () => {
  ...
  const stats = computed((state) => {
    ...
    res.wins = Object.values(dailyHistory.value).filter(g => (g.findIndex((x) => x.checks.title)>=0)).length;

    const maxDaily = Math.max.apply(null, Object.keys(dailyHistory.value));

    var streak = 0;
    for(var i = maxDaily; i >= 0; i--) {
      if (i in dailyHistory.value && dailyHistory.value[i].findIndex((x) => x.checks.title)>=0) {
        streak++;
      } else {
        break;
      }
