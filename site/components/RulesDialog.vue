<template>
  <Dialog
    v-model:visible="rulesVisible" modal header="How to play" :pt="{
      mask: {
    ...
    <Fieldset legend="Example">
      <div class="exampleMain">
        ...
        <div class="exampleRight">
          <Puzzle :puzzle-mode="puzzleMode" :reveal-level="revealLevel" :card-url="cardUrl" :card-svg="cardSvg" />
        </div>
      </div>
    </Fieldset>
    <p>
...
function toggleSubtypeOverlay(event) {
  showSubtypeOverlay.value.toggle(event)
}

const showCostOverlay = ref()
function toggleCostOverlay(event) {
